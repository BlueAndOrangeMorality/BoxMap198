...
// setCameraPositions(playerPos);
	...
	/*
	 * Stellt die CameraPosition anhand von Spielerposition, Kartengröße und Bildschirmgröße ein Die Kamera folgt immer dem
	 * Spieler, bleibt aber am Rand hängen
	 */
	public Vector2 dynamicTranslate (Vector2 playerPos, Vector2 mapSize) {
		...
		if (cameraPosition.x + totalScreenSizeBox2d.x > mapSize.x) cameraPosition.x = mapSize.x - totalScreenSizeBox2d.x;

		// if the camera is at the top or bottom edge lock it to prevent a black bar
		if (cameraPosition.y < 0) cameraPosition.y = 0;
		if (cameraPosition.y + totalScreenSizeBox2d.y > mapSize.y) cameraPosition.y = mapSize.y - totalScreenSizeBox2d.y;

		// re-add center of the map
		cameraPosition.add(totalScreenSizeBox2d.cpy().scl(0.5f));

		// workaround for small maps
		if (mapSize.x < totalScreenSizeBox2d.x && mapSize.y < totalScreenSizeBox2d.y) cameraPosition.set(mapSize.cpy().scl(0.5f));
